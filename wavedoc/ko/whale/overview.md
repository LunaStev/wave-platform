...
| 오브젝트 라이브러리 | 오브젝트 구성과 ELF64 직렬화 | 재배치 가능한 오브젝트는 실행 파일이 아님 |
| IR | 구성, 출력, 검증, 스칼라 AST 변환 | 실험적 API이며 엄격한 버전 교환 형식과 텍스트 파서는 미완성 |
| 링커 | 심볼 해석 기반 기능 | 전체 재배치 적용과 실행 파일 출력은 미지원 |
